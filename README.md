...
- TensorFlow 2.16.2 (GPU version required)
- tf-keras 2.16.0
- For Training: NVIDIA GPU with at least 40GB VRAM
- For Inference: NVIDIA GPU with at least 8GB VRAM

DeepSPIRE uses TensorFlow with GPU acceleration. To ensure proper GPU support, you need a compatible NVIDIA GPU, CUDA Toolkit, and cuDNN installed on your system. For more details on installing CUDA and cuDNN, see the official NVIDIA guides and TensorFlow GPU setup. In ``scripts/bash/setup/snellius_tf_cuda_2024.sh``, we provide a setup script for the Snellius supercomputer which installs TensorFlow with CUDA and cuDNN support in a virtual environment (you will have to remove Tensorflow from the requirements.txt). You are free to adapt this when working on HPC systems. The key thing of this script is that it also enables XLA acceleration for TensorFlow which can speed up training and inference on HPCs.
