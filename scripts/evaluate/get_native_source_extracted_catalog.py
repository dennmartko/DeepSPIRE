...
def extract_catalog_from_datamap(fp: str, fwhm: float, wcs_dict: dict):
...
def get_native_source_extracted_catalog(data_maps_dirs: list, wcs_cut: dict, bands: list, fwhm_list: list, out_files: list, max_workers: int = 10):

    for band, fwhm, out_file in zip(bands, fwhm_list, out_files):
        fps = []
        for data_maps_dir in data_maps_dirs:
            pattern = os.path.join(data_maps_dir, f"*{band}*.fits")
            fps.extend([f for f in glob.glob(pattern) if 'SR' not in os.path.basename(f)])

        all_entries = []
        with progress, ProcessPoolExecutor(max_workers=max_workers) as executor:
            task = progress.add_task(f"Extracting sources for {band}...", total=len(fps))
            futures = {executor.submit(extract_catalog_from_datamap, fp, fwhm, wcs_cut): fp for fp in fps}
...
if __name__ == '__main__':
    ...
    ## You are always free to change this to your desired output file names
    ...
    get_native_source_extracted_catalog(data_maps_dirs=data_maps_dirs, wcs_cut=wcs_cut, bands=bands, fwhm_list=FWHM, out_files=out_files, max_workers = 16)