...
def plot_image_grid(
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            print(f"Processing image ({i}, {j}) with {len(cat_target)} target sources and {len(cat_sr)} SR sources.")
            ...
            print(f"Inset region at x0={x0}, y0={y0}, size={region_size}")
...
def plot_binned_iqr(S_in_list, S_out_list, bool_scatterplots, legend_labels, xlabel, ylabel, colors=None, bins=10, sigma_conf=6.8, sigma_instr = 2, save_path=None):
    """
...
def plot_completeness_reliability(input_cat, output_cat,
                                  flux_col_input, flux_col_output,
                                  bins, search_radius=4, sigma_conf=6.8, sigma_instr = 2, save_path=None):
    """
    Computes completeness and reliability across flux bins and plots them.
    """
    ...
    annotation_R = rf"R($\geq 1\sigma_{{total}}$) = {interp_R:.2f}"
    annotation_C = rf"C($\geq 1\sigma_{{total}}$) = {interp_C:.2f}"
