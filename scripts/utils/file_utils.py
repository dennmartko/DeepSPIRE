...
def get_main_dir():
    ...
    # Go up two levels to get to the main directory (since it's in src/utils/)
...
def create_model_ckpt_folder(model_name, run_name):
    results_dir = os.path.join(get_main_dir(), "results")
    model_dir = os.path.join(results_dir, model_name)
    run_name_dir = os.path.join(model_dir, run_name)
    ckpt_dir = os.path.join(run_name_dir, "checkpoints")
...
def setup_directories(config, purpose="simulation_results"):
    ...
    ckpt_dir, _ = create_model_ckpt_folder(model_name, run_name)
...
def create_log_file(model_name, run_name, first_run):
    model_log_dir = os.path.join(get_main_dir(), "results", model_name, run_name, "logs")
