...
def configure_psf_photometry(fwhm: float, threshold: float, image_data: np.ndarray, border_pad=8, fit_shape=(5,5)):
    """
    Configure and perform PSF photometry on the given image data.
    ...
    threshold : float
        Detection threshold for DAOStarFinder, in units of the image data above the background.
    image_data : numpy.ndarray
        2D numpy array containing the image data.
    border_pad : int, optional
        Padding from the image borders to exclude sources (in pixels).
        Sources closer than this to the border will be ignored. The default is 8.
    fit_shape : tuple of int, optional
        The (height, width) shape of the fitting box used for PSF fitting (in pixels).
        The default is (5, 5).

    Returns
    -------
    astropy.table.Table
        An Astropy Table containing the PSF photometry results,
        including fitted positions and fluxes. If no sources are found, returns None.

    Notes
    -----
    This function assumes a basic Gaussian PSF model, uses DAOStarFinder for initial source detection and simultaneously fits sources grouped by SourceGrouper(1.5*FWHM).
    """
    ...
    # Filter out sources too close to the image borders
    init_sources = init_sources[(init_sources["xcentroid"] >= border_pad) & (init_sources["xcentroid"] <= (image_data.shape[0]-border_pad))]
    init_sources = init_sources[(init_sources["ycentroid"] >= border_pad) & (init_sources["ycentroid"] <= (image_data.shape[0]-border_pad))]

    # ToDo: Handle case with zero sources after filtering bordering sources.
    # if len(init_sources) == 0:
    #     return None
...
def source_extraction(data: np.ndarray, wcs: WCS, file_id: int, fwhm_pix: float, threshold: float = 2e-3, border_pad: int = 8) -> pd.DataFrame:
    """
    Perform PSF photometry and return a catalog DataFrame with 'ra','dec','source_flux'.
    Returns None if no sources found.
    """
    phot_table = configure_psf_photometry(fwhm_pix, threshold, data, border_pad=border_pad)
...
def construct_source_catalog(images, wcs_dict, fwhm_pix, threshold=2e-3, border_pad=8, N_CPU=10, task_desc=None):
    ...
    with ProcessPoolExecutor(max_workers=N_CPU) as executor:
        futures = [
            executor.submit(source_extraction, image, wcs_dict[file_id], file_id, fwhm_pix, threshold, border_pad)
